...
if [[ ${PV} == 9999 ]]; then
	...
	SRC_URI="https://github.com/mpv-player/mpv/archive/fbf3ae5a84a3b9aac81258ea8996e4e647adddb8.tar.gz -> ${P}-snapshot.tar.gz"
...
src_unpack() {
	default
	mv "${WORKDIR}"/mpv-fbf3ae5a84a3b9aac81258ea8996e4e647adddb8 "${S}"
}

src_configure() {
	# My Patch
	ewarn "manually re-run swift related commands"
	ewarn "with or without including prefix include dirs to mitigate the build error"
	if use !debug; then
		if use test; then
